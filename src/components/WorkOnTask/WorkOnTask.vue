...
export default {
  ...
  async mounted() {
    ...
    await this.$api.post.searchUsers(data, token)
      .then(({data}) => {
        this.selectUsersData.push(this.currentUser);

        if (this.selectUsersData.length > 0 && this.selectUsersData[0]._id) this.selectUsersData = this.selectUsersData.concat(data.users.filter((user) => this.selectUsersData[0]._id !== user._id));
        else this.selectUsersData = data.users;
      })
      .catch(e => {
        if (e.status === 401) {
          this.setToken("");
    ...
    this.$api.post.searchProjects(data, token)
      .then(({data}) => {
        ...
        if (this.model.project === null) this.model.project = this.projectId;
    ...
    onSendValue: async function() {
      ...
      if (this.isCreate) {
        ...
        let data = {
          ...
          "executor": this.model.user === null ? "" : this.model.user,
        }

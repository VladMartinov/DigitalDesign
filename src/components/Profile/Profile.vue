<template>
  <section class="profile">
    <div class="container">
      ...
      <div class="profile__wrapper" v-if="!isUserDataEmpty">
        <div class="profile-info-img">
          ...
          <div class="profile-info-img__dropdown" v-if="isDropdownAccess">
        ...
        <div class="profile-info">
          <div class="profile-info__header">
            ...
            <DropdownIcon v-if="isDropdownAccess" :buttonsInitArray="dropDownData" @ActionSelected="onSelectActionProfile"></DropdownIcon>
          </div>
...
export default {
  ...
  computed: {
    ...
    isDropdownAccess() {
      return this.isDataLoaded && (this.isAdmin || this.currentUser._id === this.userData._id);
    },
    ...
    textPicture() {
      ...
      console.log(this.userData.name.split(' ').join(''));

      for (let splitedItem in splitedName) { resultText += splitedName[splitedItem][0] }
  ...
  methods: {
    ...
    onLoadProfileData: function() {
      ...
      this.$api.post.searchUsers(data, this.getToken)
        .then(({data}) => {
    ...
    onSetPassword: function(event) {
      ...
      this.$api.put.userPass(data, this.getToken)
        .then(() => {
    ...
    onSetPicture: function(event) {
      ...
      this.$api.put.userPicture(data, event.imageType, this.getToken)
        .then(({data}) => {
